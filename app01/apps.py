...
def add_user():
    ...
    cursor.execute(sql, [username, "7", password])
    conn.commit()
...
# 链接数据库
def my_test():
    connection = pymysql.connect(host='localhost', user='root', passwd='123456', db='gupiao', port=3306, charset='utf8')

    # 为student的数据开启一个游标cur
    cur = connection.cursor()
    # 访问product表中的前10数据
    sql = 'SELECT * FROM product order by number desc limit 0,9'
    ...
    # 执行sql中的语句，即获取product表中的前10数据
    cur.execute(sql)
    # 将获取到的sql数据全部显示出来
    see = cur.fetchall()

    # 执行sql1中的语句，即获取category表中所有的数据
    cur1.execute(sql1)
    # 将获取到的sql1数据全部显示出来
    see1 = cur1.fetchall()
    ...
    # 遍历product表中的所有数据,see绑定了sql（也就是product表中的所有数据），所以需要在see中遍历
    for data in see:
        # legname.append(data[0])
        xproduct.append(data[1])  # data[1]是指product表中第二列(name)的数据
        yvalues.append(data[4])  # data[4]是指product表中第5列(number)的数据
...
def index():
    ...
    bar_chart.add_yaxis("数据1",[4,80,7,56,41])
    bar_chart.set_global_opts(title_opts=opts.TitleOpts(title="数据可视化"))
    # 渲染HTML模板并将图表嵌入其中
    return render_template("index1.html", chart=bar_chart.render_embed())
...
def show_line():
    ...
    line = (
        ...
        .add_yaxis('Series-2',[2,5,7,-5])
        .set_global_opts(title_opts=opts.TitleOpts(title="折线图示例"))
    )

    # 渲染模板并将图表对象传递给模板
    return render_template('chart_zhexian.html', chart=line.render_embed())


@app.route('/show/sandian')
def index_aa():
    # 创建散点图
    scatter3d = Scatter3D("三维散点图示例", width=800, height=600)
    data = [
        [1, 2, 3],
        [2, 3, 4],
        [3, 4, 5],
    ...
    scatter3d.add("", data)
    return render_template('sandian.html')
...
def rose_base():
    data_labels = ['中国平安', '腾讯控股', '亚马逊', '谷歌', '苹果']
    data_values = [20, 30, 40, 52, 60]
    data_labels1 = ['中国人寿', '中国太保', '安盛保险', '美国国际集团']
    data_values1 = [45, 30, 28, 43,51]
    rose = (
        Pie()
            .add(
            "",
            [list(z) for z in zip(data_labels, data_values)],
            radius=["30%", "75%"],
            center=["25%", "50%"],
            rosetype="radius",
            ...
            .add(
            "",
            [list(z) for z in zip(data_labels1,data_values1)],
            radius=["30%", "75%"],
            center=["75%", "50%"],
            rosetype="area",
    ...
    map_chart = (
        Map()
            ...
            visualmap_opts=opts.VisualMapOpts(
                ...
                textstyle_opts=opts.TextStyleOpts(color="#dddd"),
                min_=min_data,
                max_=max_data,
            ),
        )
    ...
    chinaAbsPie = (
        Pie()
            ...
            source=[
                ...
                ["金山办公", 1265, 1322, 1524, 1432, 1468, 1731],
                ["科大讯飞", 732, 905,847, 1345, 1265,1154],
                ["同花顺", 645.8, 672.6, 795.4, 723.9, 806.4, 817.3],
                ["360", 698.7, 705.4, 692.8, 777.6, 789.5, 722.4],
            ]
        )
            .add(
            series_name="2019",
            ...
            radius=60,
            center=["25%", "30%"],
            encode={"itemName": "product", "value": "2019"},
        )
            .add(
            series_name="2020",
            data_pair=[],
            radius=60,
            center=["75%", "30%"],
            encode={"itemName": "product", "value": "2020"},
        )
            .add(
            series_name="2021",
            data_pair=[],
            radius=60,
            center=["25%", "75%"],
            encode={"itemName": "product", "value": "2021"},
        )
            .add(
            series_name="2022",
            data_pair=[],
            radius=60,
            center=["75%", "75%"],
            encode={"itemName": "product", "value": "2022"},
        )

            .set_global_opts(
            title_opts=opts.TitleOpts(title="热门股的市值-饼图", subtitle="Pie"),
...
def hist_base():
    histogram = (
        Bar()
            ...
            .add_yaxis("商家B", [42, 32, 125, 134, 117, 162, 173], category_gap=0, color='#0000FF')
...
def box_base():
        v1 = [
            ...
            [360, 540, 460, 540, 490, 480, 450, 680, 700, 640, 730, 390],
        ]
        v2 = [
            [890, 810, 810, 820, 800, 770, 760, 740, 750, 760, 910, 920],
            [890, 840, 780, 810, 760, 810, 790, 810, 820, 850, 870, 870],
            [450, 340, 660, 830, 480, 460, 550, 480, 580, 660, 630, 690],
        ]
        v3 = [
            [580, 620, 660, 540, 740, 710, 750, 740, 830, 560, 710, 520],
            [390, 810, 720, 510, 460, 510, 690, 710, 820, 690,790, 670],
            [860, 350, 480, 540, 680, 740, 630, 690, 540, 660, 630, 690],
        ]
        c = Boxplot()

        c.add_xaxis(["9月15日", "9月16日","9月17日"])
        c.add_yaxis("中国国际航空股份有限公司", c.prepare_data(v1))
        c.add_yaxis("中国南方航空股份有限公司", c.prepare_data(v2))
        c.add_yaxis("洪都航空", c.prepare_data(v3))
...
def spChart_base():
    Area_map = (
        Line()
            .add_xaxis(["五天前", "四天前", "三天前","两天前", "一天前"])
            .add_yaxis("九联科技", [7526, 7025, 6897, 7235, 7247], is_smooth=True)  # Faker.values()
            .add_yaxis("中巨芯-U", [6842, 6983, 7005, 6578, 6352], is_smooth=True)  # Faker.values()
            .add_yaxis("仕佳光子", [6258, 5988, 6052, 5879, 4526], is_smooth=True)
            .add_yaxis("和辉广电-U", [5932, 5879, 6215, 5564, 5478], is_smooth=True)
            .add_yaxis("中芯国际", [6073, 6255, 5988, 4985, 5324], is_smooth=True)
            .add_yaxis("晶科能源", [4936, 5029, 6325, 4777, 4785], is_smooth=True)
            .add_yaxis("海光信息", [4892, 4966, 5027, 4875, 4568], is_smooth=True)
            .add_yaxis("云从科技—UW", [4925, 4023, 3988, 4492, 4636], is_smooth=True)
            .set_series_opts(
            areastyle_opts=opts.AreaStyleOpts(opacity=0.5),
            label_opts=opts.LabelOpts(is_show=False),
        )
            .set_global_opts(
            title_opts=opts.TitleOpts(title="成交量-面积图（紧贴 Y 轴）"),
            xaxis_opts=opts.AxisOpts(
...
def Scatter_base():
    Scatte = (
        Scatter()
            .add_xaxis(["9月1日", "9月2日", "9月3日", "9月4日", "9月5日", "9月6日", "9月7日","9月8日","9月9日","9月10日","9月11日","9月12日","9月13日","9月14日","9月15日"])
            .add_yaxis("源杰科技", [0.57, 0.26, 2.87, -1.96, -3.42, 4.52, 0.68,0.96,1.02,-0.23,2.04,1.05,0.88,1.25,0.28])
            .add_yaxis("富信科技", [0.45, -1.23, 1.25, 0.42, 0.56, -1.25, 2.24,2.56,0.25,-1.5,0.77,0.23,-0.56,0.28,0.65])
            .add_yaxis("盛科通信-U", [-0.79,0.23,0.33,0.67,-1.23,0.79,0.22,1.56,-3.52,0.23,0.95,-1.25,2.36,3.65,-1.05])
            .set_global_opts(
            title_opts=opts.TitleOpts(title="股票相较于前一天的涨幅"),
...
def line_chart_base():
    week_name_list = ["9月1日", "9月2日", "9月3日", "9月4日", "9月5日", "9月6日", "9月7日","9月8日","9月9日","9月10日","9月11日","9月12日","9月13日","9月14日","9月15日","9月16日","9月17日","9月18日"]
    high_temperature = [345, 347, 348, 358, 348, 258, 358,360,369,378,374,383,390,381,390]
    low_temperature = [285, 340, 294, 305, 333, 129, 318,345,355,363,370,342,387,364,359]

    high_temperature_predict = [345, 347, 348, 358, 548, 258, 358,360,369,378,374,383,390,381,390,385.634,387.943,391.304]
    low_temperature_predict = [285, 340, 294, 305, 333, 129, 318,345,355,363,370,342,387,364,359,370.206,375.505,380.644]
    ...
    Line_chart1 = (
        ...
        .add_yaxis(
            ...
            .set_global_opts(
            title_opts=opts.TitleOpts(title="股票预测"),
...
def bar_chart_base():
    ...
    Uniqualitative_variable_graph = (
        Bar()
            .add_xaxis(['九联科技', '中巨芯-U', '仕佳光子', '和辉光电-U', '中芯国际', '晶科能源', '海光信息','云从科技-UW'])  # Faker.choose()
            .add_yaxis("前一天",[7247,6352,4526,5478,5324,4785,4568,4636])  # Faker.values()
            .add_yaxis("前两天",[7235,6578,5879,5564,4985,4777,4875,4492])  # Faker.values()
            .add_yaxis("前三天",[6897,7005,6052,6215,5988,6325,5027,3988])
            .add_yaxis("前四天",[7025,6983,5988,5879,6255,5029,4966,4023])
            .add_yaxis("前五天",[7526,6842,6258,5932,6073,4936,4892,4925])
            .set_global_opts(
            title_opts=opts.TitleOpts(title="成交量-柱形图", subtitle="Bar-Brush"),
...
def kline_chart():
    ...
    kline = (
        Kline()
        .add_xaxis(["2019/7/{}".format(i + 1) for i in range(31)])  # 设置 x 轴数据
        .add_yaxis("Kline", data1)  # 设置 y 轴数据
        .set_global_opts(
            xaxis_opts=opts.AxisOpts(is_scale=True),  # 设置 x 轴为数值轴
            yaxis_opts=opts.AxisOpts(is_scale=True),  # 设置 y 轴为数值轴
            title_opts=opts.TitleOpts(title="K 线图"),  # 设置标题
        )
    )
    return kline
...
if __name__ == '__main__':
    app.run(host='localhost', port=778)
